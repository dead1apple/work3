    ...
    routes: [
      {
        ...
        children: [
          ...
          { path: 'coupons', name: 'merchant-coupons', component: EmptyView },
...
describe('MerchantLayout', () => {
  it('shows only implemented navigation and separates merchant from shop', async () => {
    ...
    expect(navigation.findAll('a').map((link) => link.text())).toEqual(['首页', '商品列表', '订单管理', '优惠券管理', '店铺管理'])
    ...
    expect(navigation.get('a[href="/merchant/coupons"]').exists()).toBe(true)
