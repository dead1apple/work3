        ...
        <article v-for="order in orders" :key="order.orderNo" class="order-card">
          ...
          <div class="order-card-body">
            ...
            <div class="order-actions">
              <el-button v-for="action in getOrderActions(order.status)" :key="action" :type="actionType(action)" :plain="action !== 'pay' && action !== 'receive'" :loading="operatingKey === `${order.orderNo}:${action}`" @click="handleAction(action, order)">
                {{ actionLabel[action] }}
              </el-button>
            </div>
          </div>
...
function actionType(action) { return action === 'pay' || action === 'receive' ? 'danger' : '' }
