  ...
  <main class="detail-page">
    ...
    <template v-else-if="order">
      <section class="status-card">
        ...
        <ol class="order-flow" aria-label="订单进度">
          <li v-for="(step, index) in orderFlow" :key="step.title" :class="flowClass(index)">
            <span class="flow-dot" aria-hidden="true">
              <el-icon v-if="flowClass(index).done"><Check /></el-icon>
              <i v-else-if="flowClass(index).current" class="flow-current-dot"></i>
            </span>
            <strong class="flow-title">{{ step.title }}</strong>
            <time v-if="step.time" class="flow-time" :title="step.time">{{ shortTime(step.time) }}</time>
            <span v-else class="flow-hint">{{ flowClass(index).current ? '进行中' : '待完成' }}</span>
          </li>
        </ol>
      ...
      <OrderLogisticsMap v-if="showLogisticsMap" :order="order" />
...
import OrderLogisticsMap from './order/OrderLogisticsMap.vue'
...
const showLogisticsMap = computed(() => Boolean(order.value?.fullAddress) && ![4, 5].includes(Number(order.value?.status)))

const orderFlow = computed(() => [
  { title: '提交订单', time: order.value?.createTime || '' },
  { title: '付款成功', time: order.value?.payTime || '' },
  { title: '商品出库', time: order.value?.deliveryTime || '' },
  { title: '交易完成', time: order.value?.receiveTime || '' },
])

function flowClass(index) {
  const active = Number(order.value?.status)
  const inProgress = [0, 1, 2, 3].includes(active)
  return {
    done: index < active || (index === active && active === 3),
    current: inProgress && index === active && active !== 3,
    pending: !inProgress || index > active,
  }
}

function shortTime(time) {
  const value = String(time || '')
  return /^\d{4}-\d{2}-\d{2}/.test(value) ? value.slice(5, 16) : value
}
...
.status-card { position: relative; display: grid; grid-template-columns: 255px 1fr; min-height: 190px; border: 1px solid #e8e8e8; background: #fff; }.status-copy { display: flex; align-items: center; justify-content: center; flex-direction: column; padding: 28px; border-right: 1px solid #eee; text-align: center; }.status-copy span { color: #999; font-size: 13px; }.status-copy h1 { margin: 12px 0 8px; color: #e1251b; font-size: 26px; }.status-copy p { margin: 0; color: #777; font-size: 13px; line-height: 1.7; }.status-copy.is-success h1 { color: #28a745; }.status-copy.is-info h1 { color: #777; }
.order-flow { display: flex; align-self: center; width: 100%; margin: 0; padding: 26px 48px 30px; list-style: none; }.order-flow li { position: relative; display: flex; flex: 1; flex-direction: column; align-items: center; gap: 7px; min-width: 0; text-align: center; }
.order-flow li:not(:first-child)::before { content: ''; position: absolute; top: 15px; right: 50%; z-index: 0; width: 100%; height: 3px; border-radius: 2px; background: #ececec; }
.order-flow li.done:not(:first-child)::before, .order-flow li.current:not(:first-child)::before { background: linear-gradient(90deg, #e1251b, #ff6b62); }
.flow-dot { position: relative; z-index: 1; display: grid; width: 32px; height: 32px; place-items: center; border: 2px solid #dcdcdc; border-radius: 50%; color: #fff; background: #fff; font-size: 16px; transition: all .25s ease; }
.order-flow li.done .flow-dot { border-color: #e1251b; background: linear-gradient(135deg, #e1251b, #ff6b62); box-shadow: 0 4px 10px rgba(225, 37, 27, .25); }
.order-flow li.current .flow-dot { border-color: #e1251b; }
.flow-current-dot { width: 12px; height: 12px; border-radius: 50%; background: linear-gradient(135deg, #e1251b, #ff6b62); animation: flow-pulse 1.6s ease-out infinite; }
@keyframes flow-pulse { 0% { box-shadow: 0 0 0 0 rgba(225, 37, 27, .35); } 100% { box-shadow: 0 0 0 9px rgba(225, 37, 27, 0); } }
.flow-title { color: #333; font-size: 14px; }
.order-flow li.done .flow-title { color: #e1251b; font-weight: 700; }
.order-flow li.current .flow-title { color: #e1251b; }
.order-flow li.pending .flow-title { color: #9a9a9a; font-weight: 400; }
.flow-time { color: #999; font-size: 12px; }
.flow-hint { color: #b9b9b9; font-size: 12px; }
.order-flow li.current .flow-hint { color: #e1251b; }
.top-actions { position: absolute; right: 28px; bottom: 21px; }.top-actions :deep(.el-button--danger:not(.is-plain)) { --el-button-bg-color: #e1251b; --el-button-border-color: #e1251b; }
...
@media (max-width: 850px) { .detail-page { width: min(100% - 20px, 760px); }.status-card { grid-template-columns: 1fr; }.status-copy { border-right: 0; border-bottom: 1px solid #eee; }.order-flow { width: 100%; padding: 28px 15px 64px; }.top-actions { right: 16px; bottom: 15px; }.detail-grid { grid-template-columns: 1fr; }.info-card { min-height: auto; border-right: 0; border-bottom: 1px solid #eee; }.goods-table-head { display: none; }.goods-row { grid-template-columns: 70px 1fr auto; gap: 8px; padding: 14px; text-align: right; }.goods-product { grid-column: 1 / -1; grid-template-columns: 64px 1fr; }.goods-image { width: 64px; height: 64px; }.goods-row > span:nth-of-type(1)::before { content: '单价 '; color: #999; }.goods-row > span:nth-of-type(2)::before { content: '数量 '; color: #999; }.goods-row .subtotal { grid-column: 3; }.no-action { display: none; } }
@media (max-width: 480px) { .order-flow { padding-right: 4px; padding-left: 4px; }.flow-title { font-size: 12px; }.flow-time, .flow-hint { font-size: 11px; }.flow-dot { width: 28px; height: 28px; font-size: 14px; }.order-flow li:not(:first-child)::before { top: 13px; }.goods-card > header { align-items: flex-start; flex-direction: column; justify-content: center; gap: 5px; }.amount-summary { justify-content: stretch; }.amount-summary div { grid-template-columns: 1fr auto; } }
</style>
