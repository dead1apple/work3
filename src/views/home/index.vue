...
const categoryList = [
  { name: '超市', icon: ShoppingCart, tone: 'orange' },
  { name: '生鲜', icon: Goods, tone: 'green' },
  { name: '数码', icon: Monitor, tone: 'blue' },
  { name: '家电', icon: Refrigerator, tone: 'purple' },
  { name: '服饰', icon: ShoppingBag, tone: 'pink' },
  { name: '美妆', icon: Brush, tone: 'rose' },
  { name: '母婴', icon: Present, tone: 'yellow' },
  { name: '家居', icon: House, tone: 'cyan' },
  { name: '运动', icon: Basketball, tone: 'lime' },
  { name: '图书', icon: Reading, tone: 'indigo' },
]
  ...
  <div class="home-page">
    ...
    <template v-else>
      ...
      <section class="category-section" aria-labelledby="category-title">
        ...
        <el-row :gutter="12" class="category-grid">
          <el-col v-for="category in categoryList" :key="category.name" class="category-col" :span="4">
            <button class="category-item" type="button"><span class="category-icon" :class="`tone-${category.tone}`"><component :is="category.icon" /></span><span>{{ category.name }}</span></button>
          </el-col>
        </el-row>
      </section>
        ...
        <el-row v-else :gutter="16" class="product-grid">
          <el-col v-for="product in productList" :key="product.id" :xs="12" :sm="8" :md="6" :lg="6" :xl="6">
            <el-card class="product-card" shadow="hover" :body-style="{ padding: '0' }" role="link" tabindex="0" @click="goDetail(product.id)" @keydown.enter="goDetail(product.id)">
              ...
              <div class="product-info"><h3>{{ product.title }}</h3><div class="product-bottom"><p class="price"><small>￥</small>{{ product.price }}</p><button class="add-button" type="button" aria-label="查看商品详情" @click.stop="goDetail(product.id)">＋</button></div></div>
            </el-card>
          </el-col>
        </el-row>
      </section>
