...
import {
  Cellphone, Dish, Goods, Headset, Iphone, Monitor, Notebook, Refrigerator, Watch, WindPower,
    ...
    <template v-else>
      ...
      <section class="category-section" aria-labelledby="category-title">
        ...
        <el-row v-else :gutter="12" class="category-grid">
          <el-col v-for="category in categoryList" :key="category.id" class="category-col" :span="4">
            <button class="category-item" type="button" @click="goCategory(category.id)"><span class="category-icon" :class="`tone-${resolveFeaturedCategoryVisual(category.name).tone}`"><component :is="categoryIcons[resolveFeaturedCategoryVisual(category.name).icon]" /></span><span class="category-name">{{ category.name }}</span></button>
          </el-col>
        </el-row>
      </section>
