...
test('catalog navigation filters products and preserves non-Cartesian SKU behavior', async ({ page, api }) => {
  ...
  await expect(page.getByRole('heading', { name: '手机数码' })).toBeVisible()
  ...
  await page.evaluate(() => {
    window.__detailRouteReuseMarker = 'still-here'
    window.history.pushState({}, '', '/product/1002')
    window.dispatchEvent(new PopStateEvent('popstate'))
  })
  await expect(page.getByRole('heading', { name: '组件复用导航耳机' })).toBeVisible()
  await expect(page.getByText('第二个商品详情。')).toBeVisible()
  await expect(page.evaluate(() => window.__detailRouteReuseMarker)).resolves.toBe('still-here')
  await expect(page.evaluate(() => performance.getEntriesByType('navigation').length)).resolves.toBe(1)
