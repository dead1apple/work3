<template>
	<view class="home-page">
		...
		<template v-else>
			...
			<view v-if="brands.length" class="brand-section">
				<view class="section-heading"><view><text class="section-kicker">TRUSTED BRANDS</text><text class="section-title">品牌馆</text></view><view class="section-link" @tap="openBrands"><text>全部品牌</text><image src="/static/icons/chevron-right.png" mode="aspectFit" /></view></view>
				<scroll-view class="brand-scroll" scroll-x><view class="brand-row-list"><view v-for="brand in brands" :key="brand.id" class="brand-chip" @tap="openBrand(brand)"><view><image v-if="brand.logo" :src="brand.logo" mode="aspectFit" /><text v-else>{{ brand.name.slice(0,1) }}</text></view><text>{{ brand.name }}</text></view></view></scroll-view>
			</view>
...
<script>
	...
	import { flattenCategories, normalizeBrandList, normalizeCategoryTree, normalizeProductList } from '../../utils/normalizers.js'

	export default {
		components: { ProductCard, StateView },
		data() { return { loading: true, categories: [], brands: [], products: [] } },
		...
		methods: {
			async loadData(fromPullDown = false) {
				...
				const [categoryResult, productResult, brandResult] = await Promise.allSettled([getCategoryTree(), getProducts({ page: 1, size: 12 }), getBrands()])
				...
				if (brandResult.status === 'fulfilled') this.brands = normalizeBrandList(brandResult.value).slice(0, 10)
				this.loading = false
			...
			openBrands() { uni.navigateTo({ url: '/pages/brands/index' }) },
			openBrand(brand) { uni.navigateTo({ url: `/pages/brands/detail?id=${brand.id}` }) },
...
<style scoped>
	...
	.category-section,.brand-section,.product-section { margin-top: 18rpx; padding: 28rpx; background: #fff; }.section-heading { display: flex; align-items: flex-end; justify-content: space-between; margin-bottom: 26rpx; }.section-kicker,.section-title { display: block; }.section-kicker { margin-bottom: 7rpx; color:#e1251b; font-size:18rpx; font-weight:800; }.section-title { color:#22262d; font-size:34rpx; font-weight:800; }.section-link{display:flex;align-items:center;gap:4rpx;color:#9299a3;font-size:23rpx}.section-link image{width:25rpx;height:25rpx}.section-note { color:#9299a3; font-size:23rpx; }.brand-scroll{width:100%;white-space:nowrap}.brand-row-list{display:flex;gap:14rpx}.brand-chip{display:flex;width:150rpx;flex:0 0 150rpx;align-items:center;flex-direction:column}.brand-chip>view{display:flex;width:112rpx;height:82rpx;align-items:center;justify-content:center;overflow:hidden;border:1rpx solid #e8eaed;border-radius:8rpx;color:#e1251b;background:#fafbfc;font-size:28rpx;font-weight:800}.brand-chip image{width:82%;height:82%}.brand-chip>text{width:100%;margin-top:9rpx;overflow:hidden;color:#59616b;font-size:20rpx;text-align:center;text-overflow:ellipsis;white-space:nowrap}
